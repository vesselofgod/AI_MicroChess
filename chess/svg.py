...
def board(board=None, squares=None, flipped=False, coordinates=True, lastmove=None, check=None, arrows=(), size=None, style=None):
    ...
    for arrow in arrows:
        if arrow.color == 'r':
            ...
            color = "#888"
        opacity = '{:.3f}'.format(max(0.1, arrow.opacity))

        tail_file = chess.square_file(arrow.tail)
        tail_rank = chess.square_rank(arrow.tail)
        head_file = chess.square_file(arrow.head)
        head_rank = chess.square_rank(arrow.head)
        ...
        # svg.append(_text(opacity, xhead-21, yhead-10, SQUARE_SIZE, margin))
        ...
        if (head_file, head_rank) == (tail_file, tail_rank):
            ...
            svg.append(_text(arrow.msg, shaft_x-21, shaft_y, SQUARE_SIZE, margin))

    return ET.tostring(svg).decode("utf-8")


def tree(root=None, size=None):
    ...
    def value_to_hex(v):
        r = int(255 * (2 * v if v < 0.5 else 1.0))
        g = int(255 * (2 * v if v < 0.5 else -2 * v + 2))
        b = int(255 * (1.0 if v < 0.5 else -2 * v + 2))
        return u'#{:02x}{:02x}{:02x}'.format(r, g, b)

    def draw_line(x1, y1, x2, y2, value, depth):
        init_line_width = 4
        line_width = min(1, init_line_width / (2 ** depth))
        opacity = max(1.0, init_line_width / (2 ** depth))
    ...
    if root:
        q = list()
        q.append((20, size-20, 0, root))

        while q:
            left_bound, right_bound, depth, parent = q.pop(0)
            for i, (action, child) in enumerate(parent.children.items()):
                interval = (right_bound - left_bound) / len(parent.children)
                child_left_bound = left_bound + i * interval
                child_right_bound = left_bound + (1 + i) * interval
                child_depth = depth + 1
                x1, y1 = (left_bound + right_bound) / 2, depth * 50 + 30
                x2, y2 = (child_left_bound + child_right_bound) / 2, child_depth * 50 + 30
                draw_line(x1, y1, x2, y2, (child.wins/child.visits + 1) / 2, depth)
                q.append((child_left_bound, child_right_bound, child_depth, child))
