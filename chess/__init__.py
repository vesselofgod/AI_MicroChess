...
STARTING_FEN = '8/8/8/knbr4/p7/8/3P4/RBNK4 w Kk - 0 1'
"""The FEN for the standard chess starting position."""

# STARTING_BOARD_FEN = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR"
STARTING_BOARD_FEN = "8/8/8/knbr4/p7/8/3P4/RBNK4"
...
BB_CORNERS = BB_A1 | BB_D1 | BB_A5 | BB_D5
BB_CASTLING = BB_A1 | BB_D5
...
BB_BACKRANKS = BB_RANK_1 | BB_RANK_5
...
class BaseBoard(object):
    ...
    def _reset_board(self):
        self.pawns = BB_A4 | BB_D2
        self.knights = BB_B5 | BB_C1
        self.bishops = BB_C5 | BB_B1
        self.rooks = BB_D5 | BB_A1
        self.queens = BB_VOID
        self.kings = BB_A5 | BB_D1

        self.promoted = BB_VOID

        self.occupied_co[WHITE] = BB_D2 | BB_C1 | BB_B1 | BB_A1 | BB_D1
        self.occupied_co[BLACK] = BB_A4 | BB_B5 | BB_C5 | BB_D5 | BB_A5
        self.occupied = BB_D2 | BB_C1 | BB_B1 | BB_A1 | BB_D1 | BB_A4 | BB_B5 | BB_C5 | BB_D5 | BB_A5
...
class Board(BaseBoard):
    ...
    def reset(self):
        ...
        self.castling_rights = BB_CASTLING
        self.ep_square = None
    ...
    def generate_pseudo_legal_moves(self, from_mask=BB_ALL, to_mask=BB_ALL):
        ...
        if self.turn == WHITE:
            ...
            single_moves = pawns >> 8 & ~self.occupied
            double_moves = single_moves >> 8 & ~self.occupied & (BB_RANK_3 | BB_RANK_2)
        ...
        for to_square in scan_reversed(single_moves):
            ...
            if square_rank(to_square) in [0, 4]:
                yield Move(from_square, to_square, QUEEN)
                yield Move(from_square, to_square, ROOK)
                yield Move(from_square, to_square, BISHOP)
                yield Move(from_square, to_square, KNIGHT)
    ...
    def is_pseudo_legal(self, move):
        ...
        if move.promotion:
            ...
            if self.turn == WHITE and square_rank(move.to_square) != 4:
                return False
    ...
    def push(self, move):
        ...
        if piece_type == KING and not promoted:
            if self.turn == WHITE:
                ...
                self.castling_rights &= ~BB_RANK_5
        ...
        if castling:
            ...
            self._set_piece_at(B1 if self.turn == WHITE else C5, KING, self.turn)
            self._set_piece_at(C1 if self.turn == WHITE else B5, ROOK, self.turn)
            # if a_side:
            #     self._set_piece_at(B1 if self.turn == WHITE else C5, KING, self.turn)
            #     self._set_piece_at(C1 if self.turn == WHITE else B5, ROOK, self.turn)
            # else:
    ...
    def castling_shredder_fen(self):
        ...
        for square in scan_reversed(castling_rights & BB_RANK_5):
    ...
    def castling_xfen(self):
        ...
        for color in COLORS:
            ...
            backrank = BB_RANK_1 if color == WHITE else BB_RANK_5
    ...
    def clean_castling_rights(self):
        ...
        black_castling = castling & BB_RANK_5 & self.occupied_co[BLACK]

        if not self.chess960:
            # The rooks must be on a1, h1, a8 or h8.
            white_castling &= BB_A1
            black_castling &= BB_D5

            # The kings must be on e1 or e8.
            if not self.occupied_co[WHITE] & self.kings & ~self.promoted & BB_D1:
                white_castling = 0
            if not self.occupied_co[BLACK] & self.kings & ~self.promoted & BB_A5:
                black_castling = 0
    ...
    def has_chess960_castling_rights(self):
        ...
        if castling_rights & BB_RANK_1 and not self.occupied_co[WHITE] & self.kings & BB_E1:
            return True
        if castling_rights & BB_RANK_5 and not self.occupied_co[BLACK] & self.kings & BB_E8:
            return True
    ...
    def generate_castling_moves(self, from_mask=BB_ALL, to_mask=BB_ALL):
        ...
        backrank = BB_RANK_1 if self.turn == WHITE else BB_RANK_5
